...
factory_feed_gate_reason: body_over_cap:1414>1200
body_over_cap: true
body_chars_claimed: 1414
body_chars_cap: 1200
created: 2026-07-16
tags: [roadmap, genesis-mythos-master, phase-4, agency-ledger, absent-proxy, rail-persist]
para-type: Project
...
**AgencyPersistenceLedger** checkpoints dominate / absent-proxy / last rail. **AbsentProxyPolicyTable** installs proxy intents while away. **RailStatePersistence** holds DM rail cursor (session-local default).

## Scope

**In:** Ledger append+checkpoint (bindings, `proxy_policy_id`, last `edge_id`, `active_rig_id`); AbsentProxyPolicyTable; RailStatePersistence (last `rig_id`, map zoom; optional export); D-4.3-001/002/003.

**Out:** AgencyEnvelope (`4.3.1`); Glue/Coordinator (`4.3.2`); Camera3D; serializers; factory/L5; exec pins; passenger_fp (Phase 5).

## Behavior

Handoff complete → ledger append `{edge_id, pilot_state, binding_delta}` → proxy row install → optional rail export. Cross-load dominate needs ledger checkpoint (D-4.3-003). Detail → [[Phase-4-3-3-AgencyPersistenceLedger-AbsentProxy-and-RailStatePersistence-Roll-up-2026-07-16]].
...
**80%** — persistence nouns explicit. Exec-deferred — advisory. Slice **red** `body_over_cap:1414>1200` pending self recompact ≤1200. Next DFS **4.3.3** — not factory/L5.
