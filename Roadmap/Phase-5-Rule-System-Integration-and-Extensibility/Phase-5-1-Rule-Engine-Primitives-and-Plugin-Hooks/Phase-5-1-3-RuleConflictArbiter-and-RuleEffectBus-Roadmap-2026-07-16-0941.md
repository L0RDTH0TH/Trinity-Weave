...
body_over_cap: false
body_chars_claimed: 1195
body_chars_cap: 1200
body_chars_pre_recompact: 1353
created: 2026-07-16
tags:
- roadmap
...
**RuleConflictArbiter** = veto → priority → merge → overflow. **RuleEffectBus** = routes effects to 3.1 / 4.1 / 4.3 / IntentResolver. Priority/veto **5.1.2**; Frame/Primitives **5.1.1**. No Godot / factory/L5.

## Scope

**In:** Arbiter order; `rule_conflict_max_active`; channels (`world_delta`, `world_event`, `agency_transition`, `perspective_transition`, `canon_proposal`, `tone_bias`); async; DMPauseGate on `world_delta`.

**Out:** Core/Frame (`5.1.1`); Plugin/Manifest (`5.1.2`); spell (5.2); quest (5.3); DM priority (OQ-5.1-003); factory/L5.

## Behavior

Effects → Arbiter: veto → priority (lower wins) → merge → overflow `arbiter_overflow` top-N. Bus → queues; SimTickPipeline (3.1) order.
...
Channels, edges, OQs → [[Phase-5-1-3-RuleConflictArbiter-and-RuleEffectBus-Roll-up-2026-07-16]].

## Handoff

**80%** — arbiter/bus nouns explicit. Exec-deferred. Body ≤1200; slice green. Next DFS **4.3.3** — not factory/L5.
