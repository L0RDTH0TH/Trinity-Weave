...
def _copy_walk_scopes(
    ...
    for pattern in patterns:
        for src in sorted(scopes_dir.glob(pattern)):
            ...
            copied += 1
    for board_name in ("L5-AFFIRM-STATUS.md", "PIN-DERIVE-STATUS.md"):
        status = scopes_dir.parent / board_name
        if status.is_file():
            target = out_dir / board_name
            shutil.copy2(status, target)
            hashes[board_name] = _sha256_text(status.read_text(encoding="utf-8"))
            copied += 1
    if copied == 0:
...
def _copy_inspiration_ux_feedstock(
    vault_root: Path,
    project_id: str,
    out_dir: Path,
    hashes: dict[str, str],
) -> tuple[int, list[str]]:
    """Mirror Inspiration-UX-Feedstock into the main-visible pack (thickeners)."""
    warnings: list[str] = []
    src = (
        vault_root
        / "1-Projects"
        ...
        / "Roadmap"
        / "User-Story"
        / "Inspiration-UX-Feedstock"
    )
    dest = out_dir / "Inspiration-UX-Feedstock"
    if dest.exists():
        shutil.rmtree(dest)
    if not src.is_dir():
        warnings.append("inspiration_ux_feedstock_missing")
        return 0, warnings
    ...
    for p in sorted(src.rglob("*")):
        ...
        hashes[f"Inspiration-UX-Feedstock/{rel.as_posix()}"] = _sha256_text(text)
        copied += 1
    if copied == 0:
        warnings.append("inspiration_ux_feedstock_empty")
    return copied, warnings
...
def emit_catalog_mint_pack(
    ...
    *,
    ...
    # Same-width child batch board (Grok alignment — locked vs active parent)
    batch_status = paths["catalog"].parent / "CHILD-BATCH-STATUS.md"
    if batch_status.is_file():
    ...
    insp_count, insp_warn = _copy_inspiration_ux_feedstock(vault_root, pid, out_dir, hashes)
    warnings.extend(insp_warn)
    if insp_count == 0:
        warnings.append("inspiration_ux_feedstock_empty")
