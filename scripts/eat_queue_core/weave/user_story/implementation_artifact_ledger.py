"""Half B implementation factory artifact ledger — weld receipts separate from Half A."""
...
def ledger_path(vault_root: Path, project_id: str) -> Path:
    return (
        ...
        / project_id
        / "Factory-DRB"
        / "implementation-factory-artifact-ledger.jsonl"
    )
...
def append_implementation_ledger_event(
    vault_root: Path,
    project_id: str,
    event: dict[str, Any],
) -> Path:
    ...
    row = {**event, "recorded_at": _utc_iso(), "ledger": "implementation_factory"}
    with path.open("a", encoding="utf-8") as fh:
        fh.write(json.dumps(row, ensure_ascii=False) + "\n")
    return path


def record_implementation_artifact(
    vault_root: Path,
    project_id: str,
    *,
    artifact_path: str,
    ...
    append_implementation_ledger_event(vault_root, project_id, event)
    return event


@dataclass
class ImplementationLedgerDriftResult:
    ok: bool
...
def reconcile_implementation_ledger_drift(
    vault_root: Path,
    project_id: str,
    *,
    max_lines: int = 50,
) -> ImplementationLedgerDriftResult:
    vault_root = vault_root.resolve()
    path = ledger_path(vault_root, project_id)
    if not path.is_file():
        return ImplementationLedgerDriftResult(True)
    ...
    return ImplementationLedgerDriftResult(ok=len(drift) == 0, drift_codes=drift, last_event=last)
