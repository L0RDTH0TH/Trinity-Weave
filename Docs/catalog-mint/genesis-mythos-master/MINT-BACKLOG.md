...
frozen_at: 2026-08-01T21:11:08Z
...
- [ ] `ux_world_generation` — DM can create (table can shape) a persistent living world (`pending`) [living_world] [series]
- [ ] `ux_dm_campaign_creation` — DM can bootstrap a campaign frame inside a world (`pending`) [table] [series]
- [ ] `ux_player_character_creation` — Players can author characters and submit them for DM acceptance into a world (`pending`) [table] [series]
- [ ] `ux_dm_session_prep` — DM can prep a session without leaving the collaborative table frame (`pending`) [table] [series]
- [ ] `ux_early_game` — Early play is a power band that gates world and pillar response (`pending`) [flows] [series]
- [ ] `ux_mid_game` — Mid play is a power band for lasting pressure and deeper world response (`pending`) [living_world] [series]
- [ ] `ux_late_game` — Late play is a power band for campaign crescendo, close, and character-to-world persistence (`pending`) [living_world] [series]
- [ ] `ux_mental_stat_interpretation` — Mental stats surface available read paths not only sheet numbers (`pending`) [surfaces] [series]
- [ ] `ux_collaborative_table_agency` — Shared virtual-tabletop loop with character agency and DM orchestration (`pending`) [table] [series]
- [ ] `ux_quiet_between_pillars` — In-adventure quiet keeps continuous fiction between combat social and explore (`pending`) [table] [series]
