...
waived_axes: []
schema_version: 1
generated_at: 2026-07-27T05:29:51Z
...
- [ ] `ux_faith_shatter_at_the_machine` — Faith shatter at the machine (`pending`) [table] [phenomenology]
- [ ] `ux_fourth_option_social_agency` — Fourth-option social agency (`pending`) [table] [phenomenology]
- [ ] `ux_guide_who_was_the_problem` — Guide who was the problem (`pending`) [table] [phenomenology]
- [ ] `ux_joy_ritual_beside_betrayal_beat` — Joy ritual beside betrayal beat (`pending`) [table] [phenomenology]
- [ ] `ux_linger_after_the_fight` — Linger after the fight (`pending`) [table] [phenomenology]
- [ ] `ux_moral_fork_that_schedules_the_next_fight` — Moral fork that schedules the next fight (`pending`) [table] [phenomenology]
- [ ] `ux_party_trust_under_ascension_pressure` — Party trust under ascension pressure (`pending`) [table] [phenomenology]
- [ ] `ux_personal_stakes_set_piece_combat` — Personal-stakes set-piece combat (`pending`) [table] [phenomenology]
- [ ] `ux_post_coma_blame_texture` — Post-coma blame texture (`pending`) [table] [phenomenology]
- [ ] `ux_post_kill_political_handoff` — Post-kill political handoff (`pending`) [table] [phenomenology]
- [ ] `ux_power_object_that_demanded_a_person_not_a_check` — Power object that demanded a person, not a check (`pending`) [table] [phenomenology]
- [ ] `ux_quiet_between_pillars` — Quiet between pillars (`pending`) [table] [phenomenology]
- [ ] `ux_rehabilitation_bargain_after_betrayal` — Rehabilitation bargain after betrayal (`pending`) [table] [phenomenology]
- [ ] `ux_rescue_as_social_contract` — Rescue as social contract (`pending`) [table] [phenomenology]
- [ ] `ux_rest_as_presence_not_only_refill` — Rest as presence, not only refill (`pending`) [table] [phenomenology]
- [ ] `ux_sibling_panic_in_the_quiet_after_the_roll` — Sibling panic in the quiet after the roll (`pending`) [table] [phenomenology]
...
- [ ] `ux_friend_who_was_the_conspiracy` — Friend who was the conspiracy (`pending`) [living_world] [phenomenology]
- [ ] `ux_living_world_remembers_without_a_script` — Living-world remembers without a script (`pending`) [living_world] [phenomenology]
- [ ] `ux_quiet_level_acknowledgment` — Quiet level acknowledgment (`pending`) [living_world] [phenomenology]
- [ ] `ux_campfire_identity_chrome` — Campfire identity chrome (`pending`) [surfaces] [phenomenology]
- [ ] `ux_chrome_that_grew_with_you` — Chrome that grew with you (`pending`) [surfaces] [phenomenology]
- [ ] `ux_diegetic_chrome_persistence` — Diegetic chrome persistence (`pending`) [surfaces] [phenomenology]
- [ ] `ux_feedback_anchored_to_the_body` — Feedback anchored to the body (`pending`) [surfaces] [phenomenology]
- [ ] `ux_logistics_of_a_missing_seat` — Logistics of a missing seat (`pending`) [surfaces] [phenomenology]
- [ ] `ux_new_toy_in_the_next_fight` — New toy in the next fight (`pending`) [surfaces] [phenomenology]
- [ ] `ux_power_trophy_that_the_table_can_feel` — Power trophy that the table can feel (`pending`) [surfaces] [phenomenology]
- [ ] `ux_region_map_for_a_social_beat` — Region map for a social beat (`pending`) [surfaces] [phenomenology]
- [ ] `ux_title_reclaim_as_identity_chrome` — Title reclaim as identity chrome (`pending`) [surfaces] [phenomenology]
- [ ] `ux_verb_first_soft_exit` — Verb-first soft exit (`pending`) [surfaces] [phenomenology]
...
### `ux_quiet_between_pillars` — Quiet between pillars
...
- ux_axis: agency
- dimension: ui_surface
- summary: Designed downtime that keeps continuous fiction alive.
- pillar_notes: 
- conceptual_pin: 1-Projects/genesis-mythos-master/Roadmap/User-Story/Actual-Play-Feedstock/moments/BG-P02-camp-social-layer.md
- derived_from: actual_play:1-Projects/genesis-mythos-master/Roadmap/User-Story/Actual-Play-Feedstock/moments/BG-P02-camp-social-layer.md
- ux_family: 
