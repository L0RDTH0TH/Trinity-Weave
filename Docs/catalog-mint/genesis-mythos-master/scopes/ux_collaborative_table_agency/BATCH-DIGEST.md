...
|----|--------|----------|-------|-------------------------|------------------------|---------|
| `ux_application_shell` | pending | yes | 2 | Sparse region map vs denser chrome stacking; Shared shell for both seats vs seat-aware region packs (still one product) |  | Baseline shell — screen regions, chrome placement, layout mapping for any product (under Shared virtual-tabletop loop with character agency and DM orchestration). |
| `ux_primary_navigation` | pending | yes | 2 | Menu/route-primary vs spatial/world wayfinding-primary; Flat top-level destinations vs deep hierarchical breadcrumbs |  | How users move between major areas — menus, routes, breadcrumbs, spatial wayfinding (under Shared virtual-tabletop loop with character agency and DM orchestration). |
