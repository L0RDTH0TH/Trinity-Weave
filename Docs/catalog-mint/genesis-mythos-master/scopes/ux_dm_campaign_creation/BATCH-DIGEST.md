...
|----|--------|----------|-------|-------------------------|------------------------|---------|
| `ux_session0_bootstrap` | pending | yes | 2 | Thin collaborative seed vs DM-solo then reveal; Minimal bounds-only vs deep intent+canon pass |  | In-tool session 0 — bounds, tone pick, intent propose, table accept/revise (under DM can bootstrap a campaign frame inside a world). |
| `ux_session_onboarding` | pending | yes | 2 | First-run product onboarding vs returning-session warm start; Preference-light vs identity-tone-heavy ritual |  | First-run or session-start rituals — setup, preferences, identity tone before core use (under DM can bootstrap a campaign frame inside a world). |
| `ux_tone_profile_surface` | pending | yes | 2 | Four core profiles only vs profiles + optional modifiers; Tone as soft bias vs stronger subsystem weighting (still not siloed presets) |  | How the chosen tone biases chrome, previews, and felt world without siloed presets (under DM can bootstrap a campaign frame inside a world). |
