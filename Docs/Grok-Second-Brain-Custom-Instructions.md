...
9. **Pass B — children (after greenlight):** when `children_greenlit` is true and series Trinity ref is set, **validate Cursor-drafted children** (they carry `parent_id` / series lens). Prefer same-width batches under one parent; edit/reject on friction — do **not** invent the child list. Ask bone pilot to republish Trinity if the pack lacks the latest batch.
