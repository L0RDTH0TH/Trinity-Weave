...
| ---------------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **Mint pack (walk queue)**   | `main` **→** `Docs/catalog-mint/<project_id>/`                        | **Backlog, FEED-ENVELOPE, BATCH-DIGEST, L5 affirm digests, PACK-MANIFEST, shared rubric — what to walk next**                  |
...
**Default: Normal conversation. Mint / L5 affirm only when bone pilot explicitly instructs and names** `project_id`**.**

**Catalog mint — mandatory (two-pass + L5 affirm):**

**Grok validation ladder** (see `Docs/catalog-mint/_shared/CATALOG-MINI-TRINITY.md`): (1) series individually · (2) children summaries batched · (3) planned-row L5s batched digest-first. Never affirm series L5 before Pass B children lock. No mandatory child L5 batches unless that child is a planned catalog row. Cursor drafts ahead of Grok; validate published pack only.
...
3. **Open** `main` **pack:** `Docs/catalog-mint/<project_id>/` **—** `MINT-PACK.md` **(Walk Order),** `PACK-MANIFEST.yaml`**,** `MINT-BACKLOG.yaml` **(or** `.md`**),** `FEED-ENVELOPE.yaml`**,** `CHILD-BATCH-STATUS.md`**,** `L5-AFFIRM-STATUS.md` **when Loop 2 affirm.**
...
6. **Shared law on** `main`**:** `Docs/catalog-mint/_shared/CATALOG-MINI-TRINITY.md`**,** `UX-MINT-RUBRIC.md`**,** `FRICTION-CHECK.md`**,** `SERIES-ALTITUDE-EXEMPLARS.md`**,** `CHILD-BATCH-VALIDATION.md`**,** `L5-AFFIRM-VALIDATION.md`**.**
...
**Loop 2 — L5 affirm (first-class MO; when bone pilot says L5 affirm / Loop 2 affirm):**

13. **Digest-first only.** Open `L5-AFFIRM-STATUS.md` + per-row `scopes/<row>/L5-AFFIRM-DIGEST.md`. Return **one** receipt per `L5-AFFIRM-VALIDATION.md`. Open full `scopes/<row>/L5.md` only for yellow / red / thin. Do **not** walk all full L5s. Affirm only **planned** catalog rows — not every Pass B child.
14. **Checks:** Pass-B drift vs SERIES; thin-parent moment floor; pack-content smell; PoC ≠ full vision; backend-only framing without observable moments. Soft `needs_pin` = yellow, not invent pin text.
15. **Suggest ≤3 cross-row flags** for STATUS; operator owns attest. L5 files existing ≠ signed / Loop 2 ready.
