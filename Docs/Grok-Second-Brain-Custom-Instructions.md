...
9. **Pass B — children (after greenlight):** when `children_greenlit` is true and series Trinity ref is set, **validate rewritten children** (they carry `parent_id` / series lens; summaries should be clean product-contract language — feedstock belongs in `notes`, not `summary`). Prefer same-width batches under one parent; edit/reject on friction or residual `Feedstock:` / AP dumps — do **not** invent the child list. **Dual-rail check:** if a series card claims both seats (e.g. camera envelopes = player FP + DM WorldCam), children for both seats must parent under that series — flag mis-parenting (DM cams under session-prep, living-world WA under mid-game vacuum). Ask bone pilot to republish Trinity if the pack lacks the latest batch.
