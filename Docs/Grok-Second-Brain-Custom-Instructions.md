...
| ---------------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **Mint pack (walk queue)**   | `main` **→** `Docs/catalog-mint/<project_id>/`                        | **Backlog, FEED-ENVELOPE, BATCH-DIGEST, Inspiration-UX, PIN-DERIVE, PIN-EXCERPTS, L5 affirm digests, PACK-MANIFEST, shared rubric** |
...
3. Open pack `Docs/catalog-mint/<project_id>/` — MINT-PACK, PACK-MANIFEST, MINT-BACKLOG, FEED-ENVELOPE, Inspiration-UX-Feedstock, CHILD-BATCH-STATUS, PIN-DERIVE-STATUS, PIN-EXCERPTS, L5-AFFIRM-STATUS as needed.
...
12. World ≠ campaign.
13. Stale pack → ask Trinity republish — do not invent feedstock.
...
5. Mint gate (you own volume): yellow+locations → pass-to-Cursor (loop cap one). Yellow+no span → sparse mint_target. Dual-approve → **mint before L5**. After mint → Grok subset = **that row only**. No waive-as-proxy for approved targets. Reject → reject_reason (do not re-propose same title).
6. Grok ≠ highlight UI — pack plain excerpts only.
7. Do not draft L5 in a pin-derive turn. Board confirm → apply_pins → series L5 → children L5.

**L5 affirm (after pins + approved mints written):**

1. Digest-first: L5-AFFIRM-STATUS + digests. One receipt per L5-AFFIRM-VALIDATION. Full L5 only yellow/red/thin.
2. Series first; then children batches under each parent (all Pass B; inherit series pins). Seasoning may cite inspiration/AP derived pins — never treat game titles as Conceptual pins.
3. Checks: Pass-B drift; missing Conceptual lens / unresolved pin; thin moments; pack-smell; PoC ≠ full vision; backend-only without moments; hyper-fit to one inspiration source.
